...
const plugin = function rateLimitPlugin(userOptions) {
	...
	return function registerRateLimitPlugin(app) {
		...
		plugin$1.onBeforeHandle({ as: options.scoping }, async function onBeforeHandleRateLimitHandler({ set, request, cookie }) {
			...
			*/
			if (options.skip.length >= 2) clientKey = await options.generator(enhancedRequest, options.injectServer?.() ?? app.server, derived);
			if (await options.skip(enhancedRequest, clientKey) === false) {
				...
				*/
				...
				if (options.headers) for (const [key, value] of Object.entries(builtHeaders)) set.headers[key] = value;
				logger("plugin", "clientKey %s passed through with %d/%d request used (resetting in %d seconds)", clientKey, maxLimit - payload.remaining, maxLimit, reset);
			}
		});
		plugin$1.onError({ as: options.scoping }, async function onErrorRateLimitHandler({ request, cookie }) {
			const derived = buildDerived(arguments[0]);
			if (!options.countFailedRequest) {
				const enhancedRequest = Object.defineProperty(request, "cookie", { value: cookie });
				const clientKey = await options.generator(enhancedRequest, options.injectServer?.() ?? app.server, derived);
